...
extern void LCD (unsigned char code[]);
extern float getADC (void);
  ...
  do {
    ...
    if (var[0] != 0) {
      ...
      if (str_scomp (var, "led0=on") == __TRUE) {
        ...
        stpassw |= 2;
      }else if (str_scomp (var, "text=") == __TRUE) {
				str_copy (text, var+5);
				LCD(text);
			}
    }
...
U16 cgi_func (U8 *env, U8 *buf, U16 buflen, U32 *pcgi) {
  ...
  switch (env[0]) {
    ...
    case 'e':
      ...
      else if (strcmp ((const char *)lang, "fa") == 0) {
        lang = "Farsi/Persian";
      }
    ...
    case 'y':
      ...
      break;
		case 'k':
			len = sprintf((char *)buf,(const char *)&env[2],a);
    case 'l' :
      len = sprintf((char *)buf,"%s",text);
			break;
    case 'n' :
			adcVoltage=getADC();
      len = sprintf((char *)buf,"%f",adcVoltage);
			break;
